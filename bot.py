...
ORIGINAL_TOPICS = [
    ...
    "лайфхаки для дома (бытовые хитрости, которые экономят время и деньги)",
    "инструменты и техника (что реально нужно, а что маркетинговая разводка)",
    "экономия на ремонте и стройматериалах, где не стоит экономить",
    "частые ошибки при ремонте и стройке — истории и разбор",
    "тренды и мода в интерьере, что уже устарело",
    "уход за домом и бытовой техникой, чтобы дольше служила",
    "мебель и хранение (организация пространства, маленькие квартиры)",
    "освещение в доме (свет, который меняет восприятие комнаты)",
...
TONE_OF_VOICE = (
    "Тон канала: с юмором и лёгкой иронией, но по делу — читатель должен и улыбнуться, "
    "и реально получить пользу. Обращайся на «ты». Аудитория смешанная, мужчины и женщины "
    "примерно поровну — не заигрывай в сторону только одного пола. "
    ...
    "ФОРМАТ ЧТЕНИЯ: пост должен читаться за 10-15 секунд, не дольше. Первая строка — конкретный "
    "цепляющий хук по делу (без вступлений и раскачки). Дальше 3-5 КОРОТКИХ предложений, "
    "по одной мысли на предложение. Если используешь сравнение или метафору — она должна быть "
...
GROQ_MODEL = "llama-3.3-70b-versatile"  # бесплатный тариф: 1000 запросов/день, 30 в минуту — заметно лучше по качеству текста, чем 8b
